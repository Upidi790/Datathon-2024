...
url = "https://journeynorth.org/sightings/querylist.html?season=fall&map=monarch-egg-fall&year=2019&submit=View+Data"
...
if response.status_code == 200:
    ...
    with open('egg2019.csv', 'w', newline='', encoding='utf-8') as csvfile:
    ...
    print("Data has been successfully scraped and saved to 'egg2019.csv'.")
else:
    print(f"Failed to retrieve the page. Status code: {response.status_code}")

data = pd.read_csv('egg2019.csv')

plt.figure(figsize=(12, 6))
plt.bar(data['State/Province'], data['Number of eggs'], color='blue')
plt.xlabel("States")
plt.ylabel("Total Number of Eggs")
plt.title('Total Number of Monarch Eggs by State in 2019')
