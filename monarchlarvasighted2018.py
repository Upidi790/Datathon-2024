...
url = "https://journeynorth.org/sightings/querylist.html?season=fall&map=monarch-larva-fall&year=2018&submit=View+Data"
...
if response.status_code == 200:
    ...
    with open('larvasighted2018.csv', 'w', newline='', encoding='utf-8') as csvfile:
    ...
    print("Data has been successfully scraped and saved to 'larvasighted2018.csv'.")
else:
    print(f"Failed to retrieve the page. Status code: {response.status_code}")

data = pd.read_csv('larvasighted2018.csv')

plt.figure(figsize=(12, 6))
plt.bar(data['State/Province'], data['Number of Larva Sighted'], color='blue')
plt.xlabel("States")
plt.ylabel("Total Number of Larva Sighted")
plt.title('Total Number of Larva Sighted by State in 2018')
